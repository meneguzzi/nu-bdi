...
public class MakeBombWorld {
  ...
  {
	  int numberOfBombs=Integer.parseInt(args[0]);
	  ...
	  for (int i=1;i<=Integer.parseInt(args[0]);i++)
	  {
		...
		if (xUnsafe)
		{
				...
				if (positiveDir)
					p.setProperty("unsafesChangeNorm"+i,"norm(prohibition, move(D), X=="+startx+"& Y>"+starty+"Y<"+finishy+", activateNorm"+i+", deactivateNorm"+i+", noUnsafe"+i+")");
				else
